...
import Matitem from '../mats/Matitem';
import '../event/event.css';

const Mat = (props) => {
    const navigate = useNavigate();
  const context = useContext(matContext)
  const { mats, displayMats, editMat } = context;
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel">Edit Material details</h5>
...

{
        ...
        .sort((a,b) => new Date(b.date) - new Date(a.date)).map((mat,_id) => {
